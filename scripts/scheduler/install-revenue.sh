#!/bin/sh
# Render the revenue / P&L collection launchd jobs.
#
# DELIBERATELY DOES NOT `launchctl load`. This is a real financial job that makes
# live (read-only) external calls; the lead loads it by hand AFTER review. This
# script only writes the .plist files and prints the exact load commands.
#
# It renders TWO jobs:
#   1. com.omniagentos.revenue           — StartCalendarInterval 02:00 (2 AM ET,
#                                           machine TZ is ET): the authoritative
#                                           snapshot of the just-closed ET day.
#   2. com.omniagentos.revenue.hourly    — StartInterval 3600: keeps *yesterday*
#                                           fresh as late refunds/settlements land
#                                           (the store UPSERTs, so this is safe).
set -eu

SCRIPT_DIR=$(CDPATH= cd -- "$(dirname -- "$0")" && pwd)
ROOT_DIR=$(CDPATH= cd -- "$SCRIPT_DIR/../.." && pwd)
TARGET_DIR=${HOME}/Library/LaunchAgents
HOUR=${OMNIAGENTOS_REVENUE_HOUR:-2}
MINUTE=${OMNIAGENTOS_REVENUE_MINUTE:-0}
mkdir -p "$TARGET_DIR"
...
module_args = ["-m", "omniagentos.revenue.collect", "--once"]

# 1) Authoritative 2 AM ET snapshot (StartCalendarInterval template).
authoritative = render_template(
    (script_dir / "com.omniagentos.revenue.plist.template").read_text(encoding="utf-8"),
...
write_plist_atomic(target_dir / "com.omniagentos.revenue.plist", authoritative)
...
write_plist_atomic(target_dir / "com.omniagentos.revenue.hourly.plist", hourly)
PY

echo "Wrote $TARGET_DIR/com.omniagentos.revenue.plist (2 AM ET authoritative snapshot)"
echo "Wrote $TARGET_DIR/com.omniagentos.revenue.hourly.plist (hourly refresh)"
echo
echo "NOT loaded. This makes live read-only financial API calls — load after review:"
echo "  launchctl load $TARGET_DIR/com.omniagentos.revenue.plist"
echo "  launchctl load $TARGET_DIR/com.omniagentos.revenue.hourly.plist"
