...
"$PYTHON_BIN" -u - "$@" << 'PY_EOF'
"""Descriptor-pinned SQLite restore.
...
The restore writes only a caller-provided destination that must not already
exist, and it refuses the configured default/live database outright.
"""
...
def copy_database(src_file, tmp_file):
    ...
    try:
        ...
        raise BackupError("SQLite online restore failed: " + str(exc)) from exc
    finally:
        try:
...
def run(argv):
    ...
    try:
        ...
        if default is not None and default["name"] == dst_name and default["parent"] == (
            ...
            raise BackupError("Cannot overwrite default/live database '" + dst_raw + "'.")
        ...
        try:
            ...
            src_file.revalidate("immediately after the online restore")
            tmp_file.revalidate("immediately after the online restore")
...
def main():
    if len(sys.argv) != 3:
        sys.stderr.write("Usage: grok-db-restore.sh <source_backup> <destination_db>\n")
        return 1
    try:
        return run(sys.argv)
