"""Small, dependency-free renderer for the backlog-executor launchd plist.

Mirrors `scripts/fable-curator/launchd.py`'s shape (single daily
`StartCalendarInterval`). Duplicated rather than imported across sibling
`scripts/*` trees, which are not real installed Python packages (same
reasoning as scripts/swarm/launchd.py).

Render-only, deliberately: installing/loading is `install.sh`'s deploy-time
...
def render_template(
    ...
    *,
    ...
    """Fill the backlog-executor launchd template's single
